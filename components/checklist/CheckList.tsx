...
const CheckList: NextPage<Props> = ({ heading, icon, list, index }) => {
	return <motion.div
		...
		className="flex flex-col justify-center xl:px-0 pb-common"
	...
	>
		{heading ?
			<motion.h1
				className="text-center heading my-common"
				variants={animation}
			>
				{heading}
			</motion.h1>
		...
		<div className="flex justify-center">
			<div className="max-w-screen-xl grid md:grid-cols-2 lg:grid-cols-3 grid-cols-1 w-full gap-5">
				{list.map((item, index) =>
					<motion.div
						className={` flex flex-row mb-2 ${index % 2 === 1 ? 'text-primary-600' : 'text-secondary-600'}`}
